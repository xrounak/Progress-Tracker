...
export const SpreadsheetGrid: React.FC<SpreadsheetGridProps> = ({
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-border bg-card/60 shadow-soft">
      <div className="min-w-max">
        ...
        {tasks.length === 0 ? (
          <div className="px-4 py-6 text-center text-[11px] text-muted-foreground">
            No tasks yet for this goal. Add some tasks from the Goals page to
            start tracking.
          </div>
        ) : (
          tasks.map((task) => (
          ...
          <div className="flex min-w-max items-center gap-2 border-t border-border/80 bg-background/90 px-2 py-2 text-[11px] text-muted-foreground">
            ...
            <div className="flex gap-2">
              {dates.map((date) => (
                <div
                  key={date}
                  className="flex w-9 items-center justify-center text-[10px]"
                >
                  {dailyTotals[date] ?? 0}
                </div>
              ))}
